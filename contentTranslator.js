...
class ContentTranslator {
  ...
  botFrameworkFactToLivePersonElement(botFrameworkFactValue) {
    ...
    if (title && url) {
      ...
      return new RichContentDefinitions.TextElement(botFrameworkFactValue, "");
    }
  }
  /**
    ...
    const convertValues = values.map(value => {
      if (value.startsWith("DATE")) {
        ...
        if (dateOptions[1].toLowerCase() === "long") {
          return `${helpers_1.days[date.getDay()]}, ${
            helpers_1.months[date.getMonth()]
          } ${date.getDate()}${helpers_1.getDayOfMonthSuffix(
            date.getDate()
          )}, ${date.getFullYear()}`;
        } else if (dateOptions[1].toLowerCase() === "short") {
          return `${helpers_1.days[date.getDay()].slice(
            0,
            3
          )}, ${helpers_1.months[date.getMonth()].slice(
            0,
            3
          )} ${date.getDate()}${helpers_1.getDayOfMonthSuffix(
            date.getDate()
    ...
    if (type === RichContentDefinitions.ElementTypes.Container) {
      ...
      botFrameworkItem.facts.forEach(fact => {
        ...
        horizontal.elements.push(
          new RichContentDefinitions.TextElement(fact.title, "", {
            bold: true
          })
        );
        horizontal.elements.push(
      ...
      if (poster) {
        elements.push(new RichContentDefinitions.Image(poster, ""));
      }
   ...
   * @param elements LivePerson elements container.
   */
  botFrameworkActionToLivePersonElement(
    action,
    elements,
    isMultiSelect = false
  ) {
    ...
    if (type === "Action.OpenUrl") {
      ...
      elements.push(
        new RichContentDefinitions.Button("", title, [buttonAction], metadata)
      );
